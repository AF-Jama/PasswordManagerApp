...
const login = async (req,res)=>{
    ...
    try{
        ...
        res.cookie('token',token,{ expires: new Date(Date.now() + 3600000), httpOnly: true, secure: true }) // stores jwt token in httpOnly cookie which is sent in each subsequent request      
        return res.json({
            msg:"login succesful",
            statusCode:200
        }).status(200)
