...
const login = async (req,res)=>{
    ...
    try{
        ...
        res.cookie('token',token,{ expires: new Date(Date.now() + 3600000), httpOnly: true, secure: true,sameSite: 'None',domain:"http://passwordmanagerbucket.s3-website-us-east-1.amazonaws.com"}) // stores jwt token in httpOnly cookie which is sent in each subsequent request      
        return res.json({
            msg:"login succesful",
            statusCode:200
        }).status(200)
